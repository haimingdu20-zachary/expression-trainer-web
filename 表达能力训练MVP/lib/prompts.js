...
function getRealtimePrompt(text, context, customPrompt) {
  ...
  const structure = context?.structure || '观点表达 PREP';
...
训练结构：${structure}
当前阶段：${currentStage || '由你判断'}

你的职责：根据最新这段话和训练阶段，判断是否触发以下任一规则。触发了输出对应提示。都没触发输出空行。
...
- 不管错别字、不管语音识别错误`,
    ...
    user: `${contextBlock} [训练结构: ${structure}] [当前阶段: ${currentStage || '未知'}]\n\n最新一段：\n"${text.slice(-500)}"`
  };
...
/**
 * 生成一次针对当前表达的追问，用于面试、汇报和高级沟通训练。
 */
function getFollowupPrompt(text, context, customPrompt) {
  const topic = context?.topic || '未设置主题';
  const scenario = context?.scenario || '通用表达';
  const structure = context?.structure || '观点表达 PREP';
  let customBlock = '';
  if (customPrompt?.goals) customBlock += `\n训练目标：${customPrompt.goals}`;

  return {
    system: `你是中文沟通训练教练。请针对用户刚才的表达提出一个最有训练价值的追问。
场景：${scenario}
主题：${topic}
表达结构：${structure}
要求：只输出一个问题，30字以内，必须具体，优先追问结论依据、案例、数据、行动或潜在风险。不要客套，不要解释，不要连续提多个问题。${customBlock}`,
...
/**
 * 根据训练场景生成一道可直接回答的练习题。
 * 这是低频 AI 能力，失败时由前端回退到本地题库。
 */
function getQuestionPrompt(context, customPrompt) {
  const scenario = context?.scenario || '通用表达';
  const structure = context?.structure || '观点表达 PREP';
  const topic = context?.topic || '用户尚未提供主题';
  let customBlock = '';
  if (customPrompt?.goals) customBlock += `\n训练目标：${customPrompt.goals}`;

  return {
    system: `你是中文口语表达训练教练。请为用户生成一道可以立即开口回答的问题。
场景：${scenario}
表达结构：${structure}
已有主题：${topic}
要求：只输出一个问题，35字以内；问题要具体、有现实感，能让用户给出明确结论、理由、例子或行动；不要客套，不要解释，不要连续提多个问题。${customBlock}`,
    user: '请生成下一道表达训练题。'
  };
}
...
结合训练目标和表达结构，判断结论、原因、证据/案例、行动/总结是否完整。指出最明显的逻辑跳跃、重复、跑题或因果不足，并给出一条可直接执行的改进建议。
...
语气要求:直接、犀利、有建设性。像一个严格但真心关心你的教练。不要客套、不要废话。`,
    ...
    user: `训练场景：${stats.scenario || '通用表达'}
训练主题：${stats.topic || '未设置'}
训练结构：${stats.structureLabel || '观点表达 PREP'}
逻辑结构得分：${stats.logicScore ?? '--'}
语速：${stats.rate || '--'}字/分钟
停顿次数：${stats.pauseCount ?? '--'}
...
module.exports = { getRealtimePrompt, getReportPrompt, getFollowupPrompt, getQuestionPrompt };
