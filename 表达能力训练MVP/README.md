...
这是本项目第三阶段的正式产品入口，用于将 PRD 中的训练流程落地为可录音、可实时分析、可复盘的桌面应用。`frontend-prototype/` 仍保留为浏览器交互验证原型，不作为正式语音识别运行入口。

一个帮助你训练表达逻辑、思维结构和口语精准度的本地桌面应用。实时语音识别 → 结构导航 → AI反馈 → 重录对比。

## MVP 新增能力

本目录是在原项目基础上制作的组合 B + D MVP，保留原有词库，不新增词库内容：

- 训练前选择场景、主题、目标时长和表达结构
- 实时显示“结论 / 原因 / 例子 / 总结”的结构进度
- 给出下一步表达提示
- 结束后生成增强报告
- 手动请求 AI 追问
- 统计估算语速和停顿
...
详细 PRD 见 `表达能力训练MVP-PRD.md`，测试步骤见 `MVP-测试说明.md`。
