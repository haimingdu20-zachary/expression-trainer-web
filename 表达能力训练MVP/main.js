...
const { sendFeedback, sendReport, sendFollowup, sendQuestion, testConnection } = require('./lib/ai-feedback');
const { analyzeLogic } = require('./lib/logic-engine');

// 覆盖应用显示名称（菜单栏、Dock、任务栏、窗口标题）
app.setName('言序｜表达与思维训练');
...
ipcMain.handle('analyze-logic', (event, data) => {
  return analyzeLogic(data);
});
...
// AI反馈（传入customPrompt）
ipcMain.handle('get-realtime-feedback', async (event, { text, context } = {}) => {
  const settings = loadSettings();
  const providerConfig = getCurrentProviderSettings(settings);
  const customPrompt = loadCustomPrompt();
  try {
    const feedback = await sendFeedback(text, { ...settings, ...providerConfig }, customPrompt, context);
...
ipcMain.handle('get-followup-question', async (event, { text, context } = {}) => {
  const settings = loadSettings();
  const providerConfig = getCurrentProviderSettings(settings);
  const customPrompt = loadCustomPrompt();
  try {
    const question = await sendFollowup(text, context, { ...settings, ...providerConfig }, customPrompt);
...
ipcMain.handle('get-question', async (event, { context } = {}) => {
  const settings = loadSettings();
  const providerConfig = getCurrentProviderSettings(settings);
  const customPrompt = loadCustomPrompt();
  try {
    const question = await sendQuestion(context, { ...settings, ...providerConfig }, customPrompt);
    return { success: true, question };
  } catch (error) {
    return { success: false, error: error.message };
  }
